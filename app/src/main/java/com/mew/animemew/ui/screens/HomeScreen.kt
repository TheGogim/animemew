    ...
    Column(modifier = Modifier.padding(top = 24.dp)) {
        ...
        LazyRow(
            ...
            items(historyList) { history ->
                // ===== Fase 3 (v14): nuevo modelo de "En espera" =====
                ...
                //   Solo se muestra "En espera" cuando:
                //   - isAiring=true (está en emisión)
                //   - progressMs=0 (no está viendo nada ahora)
                //   - episodeNumber >= totalEpisodes (vimos el último disponible)
                //   - waitingSinceTimestamp != null (marcado formalmente en espera)
                //
                // Ya NO usamos hasNewEpisode ni nextAvailableEpisode en la UI.
                // Si hay nuevos episodios, simplemente aparecen en la lista del player
                // porque totalEpisodes se actualiza desde la bolsa.
